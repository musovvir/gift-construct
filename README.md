...
## 🔌 Автозаполнение по ссылке t.me/nft/...

В модалке добавлено поле **“Ссылка на NFT”**. Поддерживаются форматы:
- `https://t.me/nft/BlingBinky-371`
- `t.me/nft/BlingBinky-371`

Клиент пытается запросить данные по slug (`BlingBinky-371`) через ваш будущий бэкенд:

- **`GET {VITE_BACKEND_URL}/v1/nft/resolve?slug=...`**

Чтобы включить это, создайте `.env` в корне:

```bash
VITE_BACKEND_URL="https://your-domain.com"
```

Если `VITE_BACKEND_URL` не задан, клиент попробует fallback к `api.changes.tg`, но это **не гарантируется** (эндпоинты могут отличаться).

### DEV-only (не для продакшена): сторонний resolve через poso.see.tg

Если хочешь быстро потестить автозаполнение локально через `poso.see.tg`, можно задать:

```bash
VITE_POSO_API_BASE="https://poso.see.tg"
```

Ограничение: по твоим данным для `poso` обязательны **`model_name` и `num`**, поэтому без Go-сервера автозаполнение “только по ссылке” может быть неполным (модель из ссылки не извлечь).

Если `poso` в будущем потребует auth, можно добавить:

```bash
VITE_POSO_TGAUTH="...секрет..."
```

⚠️ **Не используйте `VITE_POSO_TGAUTH` в продакшен-сборке фронта.** Любая переменная `VITE_*` попадает в браузер. Правильный путь — хранить секреты на **Go-сервере** и давать фронту только ваш `VITE_BACKEND_URL`.

Подробнее про план сервера и БД: `BACKEND_PLAN.md`.

## 🧩 Go backend (MVP)

В репозитории добавлена папка `backend/` с минимальным Go API для резолва `t.me/nft/...`:
- `GET /v1/nft/resolve?slug=GiftSlug-371`

Локальный запуск и переменные окружения: `backend/README.md`.
